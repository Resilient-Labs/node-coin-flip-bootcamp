document.querySelectorAll('button').forEach(button=>button.addEventListener('click', makeReq))
...
function makeReq(e){
  ...
  fetch(`/api?student=${userChoice}`)
    ...
    .then((data) => {
      console.log(data);
      document.querySelector("#personName").textContent = `The coin flip is ${data.randomanswer}, ${data.finalanswer}`
    });
