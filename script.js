...
buttons.forEach(button => button.addEventListener('click', headsOrTails)) //utilizing a forEach loop b/c we want the value of heads or tails and not the values of heads and tails

let playerScore = 0 // this is how the game starts off every game 
let compScore = 0  // // this is how the game starts off every game 

function headsOrTails(e) {

    let playerSelected = e.target.classList; //targetting our classlist (referring back to our HTML) 
    document.querySelector('.playerSelected').innerText = playerSelected
        fetch(`/api?playerChoice=${playerSelected}`) // playerChoice is our parameter
        // playerSelected = literally what the player selected

    .then(response => response.json())
    .then(data => {
        console.log(data);
        if(data.playerWin == true){ // we're grabbing the data/property AKA "playerWin" from our our object of "let winner" in our server.js
            document.querySelector('.playerScore').innerText = playerScore // displaying player score         
            playerScore++
        } else{
            compScore++ 
            document.querySelector('.compScore').innerText = compScore // displaying computer score
        }
        document.querySelector('.coinImg').innerHTML = `<img src='imgs/${data.coinFlip}.png'>` //the image is dependent on whatever the coinFlip randomizes to 
        });
