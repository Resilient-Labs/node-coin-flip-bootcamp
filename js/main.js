...
function makeReq() {
  ...
  const userGuess = document.querySelector("#userName").value;

  fetch(`/random?guess=${userGuess}`)
    .then(response => response.json())
    .then((data) => {
      console.log(data);
      document.querySelector("#coinflip").innerText = data.random
      document.querySelector("#winorloss").innerText = data.winOrLoss
    });
