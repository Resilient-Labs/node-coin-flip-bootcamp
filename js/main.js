...
function flipCoin(){
  ...
  const headsOrTails = document.querySelector("#headsOrTails").value;

  fetch(`/api?cointoss=${headsOrTails}`)
    .then(response => response.json())
    .then((data) => { 
      console.log(data);
      document.querySelector("#playerChoice").textContent = data.playerResult
      document.querySelector("#cpuChoice").textContent = data.cpuResult
      document.querySelector("#winOrLose").textContent = data.overallResult
    });
