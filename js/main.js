...
function makeReq(){
  ...
  fetch(`/api?student=${userName}`)
    ...
    .then((data) => {
      console.log(data);
      document.querySelector("#personName").textContent = data.personChoice
      document.querySelector("#personStatus").textContent = data.flipResult
      document.querySelector("#personOccupation").textContent = data.personWinOrLoss
    });

}
//worked with leny, zain, victor
