...
function makeReq() {
  ...
  const flipinput = document.querySelector("#coinflip").value;

  fetch(`/api?coin=${flipinput}`)
    .then(response => response.json())
    .then((data) => {
      console.log(data);
      document.querySelector("#flipchoice").textContent = data.name
      document.querySelector("#coinresult").textContent = data.winOrLose
